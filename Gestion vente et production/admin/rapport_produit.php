...
<head>
    ...
    <title>Statistiques du Produit le Mieux Vendu</title>
...
<body>
<div class="container">
        <h2>Statistiques du Produit le Mieux Vendu</h2>
    ...
    <script>
        // Récupérer les données des ventes pour chaque période depuis la base de données avec PHP
        <?php
        ...
        $daily_sales_query = "SELECT DATE(Date_vente) AS Date, SUM(Quantite) AS TotalQuantite FROM ventes GROUP BY DATE(Date_vente)";
        $weekly_sales_query = "SELECT YEARWEEK(Date_vente) AS Week, SUM(Quantite) AS TotalQuantite FROM ventes GROUP BY YEARWEEK(Date_vente)";
        $monthly_sales_query = "SELECT DATE_FORMAT(Date_vente, '%Y-%m') AS Month, SUM(Quantite) AS TotalQuantite FROM ventes GROUP BY DATE_FORMAT(Date_vente, '%Y-%m')";
        $yearly_sales_query = "SELECT YEAR(Date_vente) AS Year, SUM(Quantite) AS TotalQuantite FROM ventes GROUP BY YEAR(Date_vente)";
        ...
        while ($row = $daily_sales_result->fetch_assoc()) {
            $daily_sales_data[$row['Date']] = $row['TotalQuantite'];
        }

        while ($row = $weekly_sales_result->fetch_assoc()) {
            $weekly_sales_data[$row['Week']] = $row['TotalQuantite'];
        }

        while ($row = $monthly_sales_result->fetch_assoc()) {
            $monthly_sales_data[$row['Month']] = $row['TotalQuantite'];
        }

        while ($row = $yearly_sales_result->fetch_assoc()) {
            $yearly_sales_data[$row['Year']] = $row['TotalQuantite'];
        }
        ...
        // Charger les données récupérées depuis PHP dans JavaScript
        ...
        var dailyChart = new Chart(dailyCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Ventes Journalières du Produit le Mieux Vendu',
        ...
        var weeklyChart = new Chart(weeklyCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Ventes Hebdomadaires du Produit le Mieux Vendu',
        ...
        var monthlyChart = new Chart(monthlyCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Ventes Mensuelles du Produit le Mieux Vendu',
        ...
        var yearlyChart = new Chart(yearlyCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Ventes Annuelles du Produit le Mieux Vendu',
