...
session_start();
error_reporting(0);
include('dbconnection.php'); // Correction de la syntaxe pour l'inclusion du fichier
if (strlen($_SESSION['imsaid'])==0) { // Correction de la condition
  header('location:logout.php');
} else {
    if(isset($_POST['submit'])) {
        $adminid = $_SESSION['imsaid'];
        $aname = $_POST['adminname']; // Nom de l'administrateur
        $amobno = $_POST['contactnumber']; // Numéro de téléphone de l'administrateur
        
        // Mise à jour de la requête pour correspondre aux noms des colonnes de la table `administrateur`
        $query = mysqli_query($con, "UPDATE administrateur SET nom_admin ='$aname', tel_admin='$amobno' WHERE id_admin='$adminid'");
        if ($query) {
...
<html lang="en">
<head>
<title>GESTION VENTE ET PRODUCTION Systeme || Profil</title>
<!-- Reste du code HTML et PHP pour la structure de la page et le formulaire... -->
